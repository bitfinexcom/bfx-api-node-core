...
module.exports = class Manager extends EventEmitter {
  ...
  openWS () {
    ...
    ev.on('self:reopen', this.onWSReopen.bind(this, id))
    ...
    // Data events
    ev.on('data:notification', this.onWSData.bind(this, 'id', 'notification'))
    ...
    ev.on('data:managed:book', this.onWSData.bind(this, id, 'managed:book'))
    ev.on('data:managed:candles', this.onWSData.bind(this, id, 'managed:candles'))
    ev.on('data:book:cs', this.onWSData.bind(this, id, 'bookCS'))
    ev.on('data:auth', this.onWSAuthData.bind(this, id))

    // Other events
    ev.on('event:auth', this.onWSEvent.bind(this, id, 'auth'))
    ...
    ev.on('event:info:server-restart', this.onWSEvent.bind(this, id, 'info-server-restart'))
    ev.on('event:info:maintenance-start', this.onWSEvent.bind(this, id, 'info-maintenance-start'))
    ev.on('event:info:maintenance-end', this.onWSEvent.bind(this, id, 'info-maintenance-end'))

    ev.on('plugin:event', this.onPluginEvent.bind(this, id))

    ev.on('exec:order:submit', this.onOrderSubmitEvent.bind(this, id))
    ev.on('exec:order:update', this.onOrderUpdateEvent.bind(this, id))
    ...
    this[bindType](`ws2:${eventName}`, (...eArgs) => {
      ...
      if (nextWS.then) {
        nextWS.then((res) => {
          ...
          debug('error in socket listener: %s', err.message)
        })
      } else {
        if (_isObject(nextWS)) {
  ...
  onWSAuthData (id, data) {
    ...
    this.notifyPlugins('ws2', 'auth', type, {
      id,
      data: payload
    })

    this.emit(`ws2:auth:${type}`, payload, {
      id,
  ...
  onOrderUpdateEvent (wsID, changes) {
    const ws = this.getWS(wsID)
  ...
  sampleWSI () {
    return Math.floor(Math.random() * this._wsPool.length)
  }
